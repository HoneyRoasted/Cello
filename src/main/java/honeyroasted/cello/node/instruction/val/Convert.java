...
public class Convert extends AbstractNode implements Node, BooleanValue {
    @Child
    private Node value;
    private BiFunction<Environment, CodeContext, Verification<TypeInformal>> target;

    public Convert(Node value, BiFunction<Environment, CodeContext, Verification<TypeInformal>> target) {
        this.value = value;
        this.target = target;
    }
    ...
    protected Verification<TypeInformal> doVerify(Environment environment, CodeContext context) {
        ...
        if (verification.success()) {
            ...
            if (target == null || this.value.type().isAssignableTo(target)) {
                ...
                return Verification.error(this, Verify.Code.TYPE_ERROR, "%s is not assignable to %s", this.value.type(), target);
            }
        } else {
            return verification;
        }
    ...
    protected void doApply(InstructionAdapter adapter, Environment environment, CodeContext context) {
        ...
        convert(src, dst, adapter);
    }

    public static void convert(TypeInformal src, TypeInformal dst, InstructionAdapter adapter) {
        if (dst != null) {
            if (src instanceof TypeFilled srcFld && dst instanceof TypeFilled dstFld) {
                if (srcFld.isPrimitive() && dstFld.isPrimitive()) {
                    ...
                    convert(srcFld, Types.unbox(srcFld), adapter);
                    convert(Types.unbox(srcFld), dst, adapter);
