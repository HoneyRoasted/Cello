...
public class BooleanOr extends AbstractNode implements BooleanValue {
    @Child
    private Convert left;
    @Child
    private Convert right;

    public BooleanOr(Node left, Node right) {
    ...
    public void jumpIfTrue(Label ifTrue, InstructionAdapter adapter, Environment environment, CodeContext context) {
        this.left.jumpIfTrue(ifTrue, adapter, environment, context);
        this.right.jumpIfTrue(ifTrue, adapter, environment, context);
    }

    @Override
    public void jumpIfFalse(Label ifFalse, InstructionAdapter adapter, Environment environment, CodeContext context) {
        Label end = new Label();
        jump(end, ifFalse, adapter, environment, context);
        adapter.mark(end);
    }
